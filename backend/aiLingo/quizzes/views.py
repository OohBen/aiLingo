...
class CreateQuizView(generics.CreateAPIView):
    ...
    def parse_generated_questions(self, text):
        ...
        for block in question_blocks:
            ...
            if 'text' in question_data and 'choices' in question_data and 'answer' in question_data and 'explanations' in question_data and 'worth' in question_data:
...
class QuizAttemptView(generics.CreateAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        quiz_count = Attempt.objects.filter(user=user, quiz__language=quiz.language).count()
        average_score = Attempt.objects.filter(user=user, quiz__language=quiz.language).aggregate(Avg('score'))['score__avg'] or 0
        topic_preferences = {}
        ...
        if analytics_data is None:
            analytics_data = {}
        analytics_serializer = AnalyticsSerializer(data={'user': user.id, 'data': analytics_data})
